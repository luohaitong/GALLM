...
from gallm.model.graph_layers import MPNN, GNN, CLIP, graph_transformer
...
class GraphLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if hasattr(config, "graph_tower"):
            if 'MPNN' in self.config.graph_tower: 
                self.graph_tower = MPNN(in_channels = self.config.graph_hidden_size, hidden_channels = self.config.graph_hidden_size * 2, out_channels = self.config.graph_hidden_size, dropout = 0.1, num_layers = 2, if_param = False)
            elif 'gcn' in self.config.graph_tower:
            ...
            elif 'gt' in self.config.graph_tower:
                clip_graph, args= load_model_pretrained(CLIP, self.config.pretrain_graph_model_path) 
    ...
    def forward(
        ...
        if (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
                ...
                if type(graph_data) is list:
                    ...
                    for g in graph_data:
                        if type(g) is Data:
                            ...
                            # else:
                            #     node_forward_out = g.graph_node
                            node_forward_out = g.graph_node
                            graph_node_features.append(node_forward_out)
                        elif type(g) is dict:
                            # if graph_tower is not None:               
                            #     node_forward_out_1 = graph_tower(g['graph_1'])
                            #     node_forward_out_2 = graph_tower(g['graph_2'])
                            # else:
                            #     node_forward_out_1 = g['graph_1'].graph_node
                            #     node_forward_out_2 = g['graph_2'].graph_node
                            node_forward_out_1 = g['graph_1'].graph_node
                            node_forward_out_2 = g['graph_2'].graph_node
...
class GraphLlamaModel_aware(LlamaModel):
    config_class = GraphLlamaConfig

    def __init__(self, config: LlamaConfig):
        super(GraphLlamaModel_aware, self).__init__(config)
        ...
        #if hasattr(config, "label_iaware"):
            self.label_soft_prompt = nn.Embedding(self.config.num_label * 3, self.config.hidden_size)
            modules = [nn.Linear(self.config.hidden_size, self.config.hidden_size)]
            modules.append(nn.ReLU())
            modules.append(nn.Linear(self.config.hidden_size, self.config.hidden_size))
            self.label_projector = nn.Sequential(*modules)
    ...
    def forward(
        ...
        if 1:    
            soft_embeds = self.obatain_soft_embeds(self.config.num_label, batch_size = input_ids.shape[0])
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            new_input_embeds = []
            cur_graph_idx = 0
            for cur_input_ids, cur_input_embeds, cur_soft_embeds in zip(input_ids, inputs_embeds, soft_embeds):
                if (cur_input_ids == graph_tower.config.graph_patch_token).sum() == 0:
                    # multimodal LLM, but the current sample is not multimodal
                    cur_input_embeds = cur_input_embeds + (0. * dummy_graph_features).sum()

                    if 1:
                        #replace label tokens with soft prompt tokens
                        num_patches = cur_soft_embeds.shape[0]
                        masked_indices = torch.where(cur_input_ids == graph_tower.config.graph_label_token)[0]
                        mask_index_start = masked_indices[0]
                        cur_input_embeds = torch.cat((cur_input_embeds[:mask_index_start], cur_soft_embeds, cur_input_embeds[mask_index_start+num_patches:]), dim=0)
                ...
                if graph_tower.config.use_graph_start_end:
                    ...
                    if 1:
                        ...
                        masked_indices = torch.where(cur_input_ids == graph_tower.config.graph_label_token)[0]
                        mask_index_start = masked_indices[0]
                        cur_new_input_embeds = torch.cat((cur_new_input_embeds[:mask_index_start], cur_soft_embeds, cur_new_input_embeds[mask_index_start+num_patches:]), dim=0)
        ...
        return super(GraphLlamaModel_aware, self).forward(
            input_ids=None, attention_mask=attention_mask, past_key_values=past_key_values,
            inputs_embeds=inputs_embeds, use_cache=use_cache,
            output_attentions=output_attentions, output_hidden_states=output_hidden_states,
            return_dict=return_dict
    ...
    def obatain_soft_embeds(self, num_label, batch_size):
        soft_label_ids = torch.arange(num_label * 3).unsqueeze(0).repeat(batch_size,1).cuda()
        soft_label_emb = self.label_soft_prompt(soft_label_ids)
        soft_label_emb = self.label_projector(soft_label_emb)
        return soft_label_emb
...
class GraphLlamaForCausalLM_iaware(LlamaForCausalLM):
    config_class = GraphLlamaConfig

    def __init__(self, config):
        super(LlamaForCausalLM, self).__init__(config)

        self.model = GraphLlamaModel_aware(config)
    ...
    def initialize_graph_tokenizer(self, use_graph_start_end, tokenizer, device,
        ...
        self.resize_token_embeddings(len(tokenizer))
        vision_config.graph_label_token = tokenizer.convert_tokens_to_ids([DEFAULT_LABEL_TOKEN])[0]
        ...
        if use_graph_start_end:
            ...
            if pretrain_graph_mlp_adapter:
                ...
                if input_embeddings.shape == embed_tokens_weight.shape:
                    ...
                    input_embeddings[-num_new_tokens:] = embed_tokens_weight[-num_new_tokens-1:-1]
                # else:
                #     raise ValueError(f"Unexpected embed_tokens_weight shape. Pretrained: {embed_tokens_weight.shape}. Current: {input_embeddings.shape}. Numer of new tokens: {num_new_tokens}.")
